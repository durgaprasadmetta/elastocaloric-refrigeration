...
with col1:
    ...
    if st.session_state.chamber_temp <= 0:
        box_style = f"background-color: #0B3C5D; padding: 20px; border-radius: 10px; text-align: center; color: white;"
    else:
        box_style = f"background-color: #1E1E1E; padding: 20px; border-radius: 10px; text-align: center; color: #FFA500;"
        
    st.markdown(f"<div style='{box_style}'><h3>Isolated Sub-Zero Vault</h3><h1>{st.session_state.chamber_temp:.1f}°C</h1><p>Target Goal: -18.0°C</p></div>", unsafe_allowed_html=True)
